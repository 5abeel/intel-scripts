...
function download() {
    ...
    wget --user=$USERNAME --ask-password $ARTIFACTORY_URL/deploy-sdk/oem_generic/intel-ipu-host-components-ts.${CI_TYPE}.${CI_NUM}.tar.gz
    wget --user=$USERNAME --ask-password $ARTIFACTORY_URL/deploy-sdk/internal_only/hw-p4-programs.ts.${CI_TYPE}.${CI_NUM}.tgz
...
function unzip_images() {
    ...
    tar -xvzf intel-ipu-host-components-ts.${CI_TYPE}.${CI_NUM}.tar.gz
    tar -xvzf hw-p4-programs.ts.${CI_TYPE}.${CI_NUM}.tgz
