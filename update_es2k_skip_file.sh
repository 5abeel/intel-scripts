...
target_interfaces=(enp0s1f0d4 enp0s1f0d5 enp0s1f0d6 enp0s1f0d7 enp0s1f0d8 enp0s1f0d9)
...
ssh $SSH_OPTIONS $IMC "ssh $SSH_OPTIONS $ACC '
    ...
    MAC_enp0s1f0d8=\$(ip link show enp0s1f0d8 | grep -oE \"([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}\" | head -1)
    MAC_enp0s1f0d9=\$(ip link show enp0s1f0d9 | grep -oE \"([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}\" | head -1)
    
    # Check if all MAC addresses were found
    if [ -z \"\$MAC_enp0s1f0d4\" ] || [ -z \"\$MAC_enp0s1f0d5\" ] || [ -z \"\$MAC_enp0s1f0d6\" ] || [ -z \"\$MAC_enp0s1f0d7\" ] || [ -z \"\$MAC_enp0s1f0d8\" ] || [ -z \"\$MAC_enp0s1f0d9\" ]; then
        echo \"Error: Could not retrieve all MAC addresses\"
        exit 1
    fi
    
    # Create new ctrl_map value
    NEW_CTRL_MAP=\"[\\\"NETDEV\\\",\\\"\$MAC_enp0s1f0d4\\\",\\\"\$MAC_enp0s1f0d5\\\",\\\"\$MAC_enp0s1f0d6\\\",\\\"\$MAC_enp0s1f0d7\\\",\\\"\$MAC_enp0s1f0d8\\\",\\\"\$MAC_enp0s1f0d9\\\",1]\"
    ...
    if jq empty \$TMP_FILE > /dev/null 2>&1; then
        ...
        echo \"MAC addresses: \$MAC_enp0s1f0d4, \$MAC_enp0s1f0d5, \$MAC_enp0s1f0d6, \$MAC_enp0s1f0d7, \$MAC_enp0s1f0d8, \$MAC_enp0s1f0d9\"
        exit 0
    else
...
if [ $? -eq 0 ]; then
    ...
    echo " >>> Use enp0s1f0d4, enp0s1f0d5, enp0s1f0d6, enp0s1f0d7, enp0s1f0d8, enp0s1f0d9 as PRs for networking setup"
    exit 0
else
